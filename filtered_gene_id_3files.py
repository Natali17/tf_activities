...
if len(sys.argv) < 4:
    print("Usage: python filtered_gene_id.py <file1> <file2> <file3>")
    sys.exit(1)
...
# Load gene ID sets from all three files
genes1 = load_gene_ids(file1)
genes2 = load_gene_ids(file2)
genes3 = load_gene_ids(file3)

# Compute intersection of gene IDs from all three files
common_genes = genes1.intersection(genes2).intersection(genes3)
print(f"Number of common gene IDs: {len(common_genes)}")

# Filter all three files based on common gene IDs and save output
...
filter_file_by_genes(file3, f"{file3.rsplit('.', 1)[0]}_filtered.tsv", common_genes)
